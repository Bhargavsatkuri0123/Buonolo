...
import { GENERATE_DUMMY_COMMUNITIES, GENERATE_DUMMY_EVENTS, GENERATE_DUMMY_PEOPLE, SAF, DUMMY_COMMUNITIES, DUMMY_EVENTS, DUMMY_PEOPLE } from "../constants";
...
export const GroupView = ({ group, onClose, T, onUserClick, onEventClick, onInviteClick, user, events }: any) => {
...
export const CommunityTab = ({ communitiesData, activeCommunityTab, setActiveCommunityTab, profile, user, T, onRefreshGroups, onStartChat }: CommunityTabProps) => {
  ...
  const handleCreateEvent = async () => {
    if (!newEventName.trim() || !newEventDate.trim() || !newEventLocation.trim() || !user) return;
    setIsCreating(true);
    setCreateError("");
    ...
    <div className="pb-24">
      <Header T={T} title="Community" right={
        <div className="flex gap-2">
          ...
          {activeCommunityTab === "groups" ? (
            ...
            <button onClick={() => setIsCreateEventOpen(true)} className={`p-2 rounded-full ${T.card}`}><Plus size={18} className={T.text} /></button>
          ) : null}
        </div>
      } />
      ...
      {isCreateEventOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4">
          <div className={`w-full max-w-sm ${T.bg} rounded-3xl overflow-hidden cardin`}>
            <div className="p-6 border-b border-orange-100 dark:border-zinc-800 flex justify-between items-center">
              <h2 className={`text-xl font-bold ${T.text}`}>Create Event</h2>
              <button onClick={() => setIsCreateEventOpen(false)} className={T.sub}><X size={24} /></button>
            </div>
            <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto no-scrollbar">
              {createError && <p className="text-xs text-red-500 font-bold bg-red-50 dark:bg-red-900/20 p-3 rounded-xl border border-red-100 dark:border-red-900/30">{createError}</p>}
              <div>
                <label className={`block text-xs font-bold uppercase tracking-wider ${T.sub} mb-1.5`}>Event Name</label>
                <input value={newEventName} onChange={e => setNewEventName(e.target.value)} placeholder="e.g. Sunday Park Picnic" className={`w-full ${T.input} rounded-xl px-4 py-3 outline-none border ${T.line}`} />
              </div>
              <div>
                <label className={`block text-xs font-bold uppercase tracking-wider ${T.sub} mb-1.5`}>Date & Time</label>
                <input value={newEventDate} onChange={e => setNewEventDate(e.target.value)} placeholder="e.g. This Sunday at 14:00" className={`w-full ${T.input} rounded-xl px-4 py-3 outline-none border ${T.line}`} />
              </div>
              <div>
                <label className={`block text-xs font-bold uppercase tracking-wider ${T.sub} mb-1.5`}>Location</label>
                <input value={newEventLocation} onChange={e => setNewEventLocation(e.target.value)} placeholder="e.g. Mauerpark" className={`w-full ${T.input} rounded-xl px-4 py-3 outline-none border ${T.line}`} />
              </div>
              <div>
                <label className={`block text-xs font-bold uppercase tracking-wider ${T.sub} mb-1.5`}>Icon / Emoji</label>
                <div className="flex flex-wrap gap-2">
                  {["📅", "🍻", "👟", "🎨", "📚", "🍳", "💻", "🌱", "🧘", "🚲"].map(emoji => (
                    <button 
                      ...
                      className={`w-10 h-10 rounded-xl text-xl flex items-center justify-center transition-all ${newEventEmoji === emoji ? "bg-orange-500 scale-110 shadow-lg" : `${T.card2} hover:bg-orange-100 dark:hover:bg-zinc-800`}`}
                    >
                      {emoji}
                    </button>
                  ))}
              ...
              <div>
                <label className={`block text-xs font-bold uppercase tracking-wider ${T.sub} mb-1.5`}>Description</label>
                <textarea value={newEventDesc} onChange={e => setNewEventDesc(e.target.value)} placeholder="What is this event about?" className={`w-full ${T.input} rounded-xl px-4 py-3 outline-none border ${T.line} h-24 resize-none`} />
              </div>
              <button 
                disabled={isCreating || !newEventName.trim() || !newEventDate.trim() || !newEventLocation.trim()} 
                onClick={handleCreateEvent}
                className="w-full bg-orange-500 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-orange-500/20 disabled:opacity-50"
              >
