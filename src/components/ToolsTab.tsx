...
import { Search, ChevronRight, Wrench, CheckCircle2, ChevronDown, MessageCircle, ExternalLink, PlayCircle, FileText } from "lucide-react";
        ...
        <div className="mx-4 space-y-6">
                ...
                {tool.details.groups.map((group: string, idx: number) => (
                  <div key={idx} className={`${T.card2} rounded-xl p-3 flex items-center gap-3 border border-orange-100 dark:border-zinc-800`}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className={`text-[11px] ${T.sub}`}>Join to ask questions</p>
                    </div>
          ...
          {!tool.details && (
            <div className="space-y-3">
              {["Checklist & required documents", "Nearby places & offices (map)", "Costs, fees & timelines", "Ask the community"].map(x => (
                <button key={x} className={`${T.card} rounded-xl p-4 w-full flex items-center justify-between text-left border border-orange-50 dark:border-zinc-800`}>
          ...
          {tool.name !== "Emergency Numbers" && (
            <button
              ...
              className="w-full bg-orange-500 text-white rounded-xl p-4 font-bold disp text-lg shadow-lg shadow-orange-500/20 active:scale-95 transition-transform"
            >
              Add to Roadmap
            </button>
          )}
  ...
  return (
    <div className="pb-24">
      ...
      <div className={`mx-4 mb-4 flex items-center gap-2 rounded-full px-4 py-2.5 ${T.card}`}>
        ...
        <input placeholder="Search tools — e.g. tax, housing, visa…" className={`flex-1 bg-transparent text-sm outline-none ${T.text}`} />
      </div>
      {toolSectionsData.map(sec => (
