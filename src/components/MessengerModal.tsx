  ...
  return (
    <div className={`fixed inset-0 z-[110] flex flex-col ${T.bg}`}>
       <div className={`sticky top-0 z-20 ${T.bg} px-4 pt-4 pb-3 flex items-center gap-3`}>
        ...
        <h1 className={`disp font-bold text-2xl ${T.text}`}>{activeMessageThread ? getThreadName(activeMessageThread) : "Messenger"}</h1>
      </div>
      {activeMessageThread ? (
        <>
          ...
          {threads.map((t: any) => (
            <button key={t} onClick={() => setActiveMessageThread(t)} className={`w-full flex items-center gap-3 p-4 hover:${T.card2}`}>
              ...
              <div className="flex-1 text-left">
                <p className={`font-bold ${T.text}`}>{getThreadName(t)}</p>
                <p className={`text-xs ${T.sub} truncate`}>{directMessages.filter((m: any) => m.threadId === t && !m.isFake).slice(-1)[0]?.text}</p>
              </div>
            </button>
          ))}
        </div>
